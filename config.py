class Config:
	...
	# Wi-Fi configuration
	WiFi_SSID = "xxx" # SSID Wi-Fi Имя сети, к которой подключаемся
	WiFi_PASSWORD = "xxx" # Password Wi-Fi пароль доступа к сети, к которой подключаемся
	WiFi_CONNECT_DELAY = 100 # Время ожидания перед подключением к сети в секундах (время для загрузки маршрутизатора)
	WiFi_CONNECT_TIMEOUT = 200 # Время ожидания подключения к сети в секундах
